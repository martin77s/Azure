...
# Get the webApp resource(s) - Main app and staging slot
$apps = @()
try {
	Write-Host ("`nSearching webApp: {0}" -f $AppServiceResourceName)
	$apps += Get-AzWebApp -ResourceGroupName $ResourceGroupName -Name $AppServiceResourceName -ErrorAction Stop
} catch {
	Write-Host ("`nNo such resource found ({0}/{1})" -f $ResourceGroupName, $AppServiceResourceName)
}

try {
	Write-Host ("`nSearching webApp slot: {0}-{1}" -f $AppServiceResourceName, $stagingSlotName)
	$apps += Get-AzWebAppSlot -ResourceGroupName $ResourceGroupName -Name $AppServiceResourceName -Slot $stagingSlotName -ErrorAction Stop
} catch {
	Write-Host ("`nNo such resource found ({0}/{1}-{2})" -f $ResourceGroupName, $AppServiceResourceName, $stagingSlotName)
}
...
if ($apps.count -gt 0) {
	...
	foreach ($app in $apps) {
		...
		$saveFlagOn = $true
		Write-Host ("`nWorking on resource: {0}" -f $app.name)

		# Build the path to the API:
		if ($app.type -eq 'Microsoft.Web/sites/slots') {
			$path = $basePath -f $subscriptionId, $app.ResourceGroup, ('{0}/slots/{1}' -f ($app.Name -split '/')), $apiVersion
		} else {
